...
def graph_epimorphism_exists(p_vertex_domains,t_edge_domains,pattern_am,target_am):
    ...
    if not t_edge_domains:
        ...
        variable = p_vertex_domains.items()[0]
#        variable = smallest_domain(p_vertex_domains.items())
        vertex_to_map = variable[0]
        poss_vertices = variable[1]
        for poss_vertex in poss_vertices:
            #print "vertices"
            # assignement: vertex_to_map -> poss_vertex
            #print "mapping ",vertex_to_map," to ", poss_vertex
            # propagation: adapt domains
            p_vertex_domains_copy = propagate_adjacency_after_vertex_assignement(p_vertex_domains,vertex_to_map,poss_vertex,pattern_am,target_am)
            # check for dead end
            if [] not in p_vertex_domains.values():
    ...
    variable = t_edge_domains[0]
 #   variable = smallest_domain(t_edge_domains)
    edge_to_map = variable[0]
    poss_edges = variable[1]
    for poss_edge in poss_edges:
        #print "mapping ", poss_edge[0]," to ",edge_to_map[0]
        #print "mapping ", poss_edge[1]," to ",edge_to_map[1]
        ...
        if [] in [variable[1] for variable in t_edge_domains_copy]:
            continue
...
def distance_matrix(am):
    dist_m = [ [0 for i in xrange(len(m))] for j in xrange(len(am))]
    for i in xrange(len(am)):
        calculate_distances(am,i,i,dist_m,1)
    return dist_m

def calculate_distances(am,orig,i,dist_m,curr_dist):
    for j in xrange(len(am)):
        if am[i][j]==1 and curr_dist[orig][j]==0:
            dist_m[i][j]==curr_dist
            if curr_dist<4:
                calculate_distances(am,orig,j,dist_m,curr_dist+1)
#block end           
#___________________________________________________________
